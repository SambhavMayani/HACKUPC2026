...
export default function Explorer({ data, campaignId, creativeId }) {
  ...
  const campaigns = useMemo(() => [...new Map(data.campaigns.map(c => [String(c.id), c])).values()], [data]);

  useEffect(() => {
    if (!campaignId) return;
    setCampaign(String(campaignId));
    setVertical('all');
    setFormat('all');
    setStatus('all');
    setSearch('');
  ...
  useEffect(() => {
    ...
    const creative = data.creatives.find(c => String(c.id) === String(creativeId));
    if (!creative) return;
  ...
  const filtered = useMemo(() => {
    ...
    if (campaign !== 'all') list = list.filter(c => String(c.campaign_id) === String(campaign));
  ...
  return (
    <div>
      ...
      <div className="filters-bar">
        ...
        <select className="filter-select" value={campaign} onChange={e => { setCampaign(e.target.value); setPage(0); }}>
          <option value="all">All Campaigns</option>
          {campaigns.map(c => <option key={c.id} value={String(c.id)}>{c.id} · {c.app}</option>)}
        </select>
        ...
        <div className="modal-overlay" onClick={() => setSelected(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: 24, marginBottom: 24, flexWrap: 'wrap' }}>
              <img src={`/api/assets/${selected.asset_file}`} alt="" style={{ width: 180, maxHeight: 320, borderRadius: 12, objectFit: 'contain', border: '1px solid var(--border)', background: 'var(--bg-secondary)' }} />
