...
export default function App() {
  ...
  const [explorerCampaignId, setExplorerCampaignId] = useState(null);
  const [explorerCreativeId, setExplorerCreativeId] = useState(null);
  const [copilotMessagesByAccount, setCopilotMessagesByAccount] = useState({});
  const [copilotInputByAccount, setCopilotInputByAccount] = useState({});
  const [copilotIncludeNewsByAccount, setCopilotIncludeNewsByAccount] = useState({});
  ...
  const copilotMessages = copilotMessagesByAccount[account] || null;
  const copilotInput = copilotInputByAccount[account] || '';
  const copilotIncludeNews = copilotIncludeNewsByAccount[account] || false;

  const openCampaignInExplorer = (campaignId) => {
    setExplorerCampaignId(String(campaignId));
    setExplorerCreativeId(null);
    setPage('explorer');
  };

  const openCreativeInExplorer = (creativeId) => {
    setExplorerCreativeId(String(creativeId));
    setExplorerCampaignId(null);
    setPage('explorer');
  };

  const pages = {
    overview: <Overview data={scopedData} insight={insight} benchmark={benchmark} onNavigate={setPage} />,
    explorer: <Explorer data={scopedData} campaignId={explorerCampaignId} creativeId={explorerCreativeId} />,
    ...
    copilot: (
      <Copilot
        ...
        onMessagesChange={messages => setCopilotMessagesByAccount(current => ({ ...current, [account]: messages }))}
        input={copilotInput}
        onInputChange={input => setCopilotInputByAccount(current => ({ ...current, [account]: input }))}
        includeNews={copilotIncludeNews}
        onIncludeNewsChange={includeNews => setCopilotIncludeNewsByAccount(current => ({ ...current, [account]: includeNews }))}
        onOpenCampaign={openCampaignInExplorer}
        onOpenCreative={openCreativeInExplorer}
      />
    ),
  };
